...
function getSampleCasingData() {
    return [
        {
            label: t("sample.casing.conductor"),
            od: 20,
            weight: 94,
            idOverride: null,
            manualHoleSize: null,
            ...
            showTop: true,
            showBottom: true,
            __i18n: { label: "sample.casing.conductor" },
        },
        {
            label: t("sample.casing.surface"),
            od: 13.375,
            weight: 54.5,
            idOverride: null,
            manualHoleSize: null,
            ...
            showTop: true,
            showBottom: true,
            __i18n: { label: "sample.casing.surface" },
        },
        {
            ...
            boc: null,
            labelXPos: null,
            manualLabelDepth: 4800,
            casingLabelFontSize: null,
            depthLabelFontSize: null,
            depthLabelOffset: null,
            showTop: true,
            showBottom: true,
            __i18n: { label: "sample.casing.intermediate" },
        },
        {
            label: t("sample.casing.production"),
            od: 7,
            weight: 29,
            idOverride: null,
            manualHoleSize: null,
            ...
            showTop: true,
            showBottom: true,
            __i18n: { label: "sample.casing.production" },
        },
        {
            label: t("sample.casing.open_hole"),
            od: 4.5,
            weight: 0,
            idOverride: null,
            manualHoleSize: null,
            ...
            showTop: true,
            showBottom: true,
            __i18n: { label: "sample.casing.open_hole" },
        },
    ];
}
function getSampleLinesData() {
    return [
        {
            depth: 2500,
            label: t("sample.line.mudline"),
            color: "steelblue",
            fontColor: "steelblue",
            fontSize: 11,
            lineStyle: translateEnum("lineStyle", "Dash-dot"),
            labelXPos: 0.9,
            show: true,
            __i18n: { label: "sample.line.mudline" },
        },
        {
            depth: 5000,
            label: t("sample.line.top"),
            color: "seagreen",
            fontColor: "seagreen",
            fontSize: 11,
            lineStyle: translateEnum("lineStyle", "Dash-dot"),
            labelXPos: 0.9,
            show: true,
            __i18n: { label: "sample.line.top" },
        },
        {
            depth: 8500,
            label: t("sample.line.target"),
            color: "tomato",
            fontColor: "tomato",
            fontSize: 11,
            lineStyle: translateEnum("lineStyle", "Dash-dot"),
            labelXPos: 0.9,
            show: true,
            __i18n: { label: "sample.line.target" },
        },
    ];
}
function getSamplePlugsData(casingData = []) {
    const casingIds = casingData.map((row, index) =>
        generateCasingId(row, index),
    );
    return [
        {
            top: 10000,
            ...
            label: t("sample.plug.label1"),
            color: "lightgray",
            hatchStyle: translateEnum("hatchStyle", "none"),
            manualWidth: null,
            show: true,
            __i18n: { label: "sample.plug.label1" },
        },
        {
            top: 13000,
            bottom: 13100,
            ...
            targetMode: translateEnum("targetMode", "Auto (innermost)"),
            attachToRow: casingIds[3] ?? casingIds[0] ?? null,
            label: t("sample.plug.label2"),
            color: "lightgray",
            hatchStyle: translateEnum("hatchStyle", "none"),
            manualWidth: null,
            show: true,
            __i18n: { label: "sample.plug.label2" },
        },
    ];
}
...
function getTrajectoryComment(row) {
    const rawComment =
        ...
        row?.notes ??
        row?.Notes;
    return rawComment === null || rawComment === undefined
        ? ""
        : String(rawComment);
}
function normalizeTrajectorySurveys(rows = []) {
    return rows
        .map((row) => {
            ...
            return { md, inc, azi, comment: getTrajectoryComment(row) };
        })
        .filter(Boolean);
}
function normalizeLegacyTrajectoryPoints(rows = []) {
    return rows
        .map((row) => {
            ...
            return { x, tvd, comment: getTrajectoryComment(row) };
        })
        .filter(Boolean);
}
function convertLegacyTrajectoryPointsToSurveys(points = []) {
    ...
    const surveys = [{ md: 0, inc: 0, azi: 0, comment: points[0].comment }];
    let cumulativeMD = 0;
        ...
        const azi =
            Math.abs(deltaX) <= EPSILON
                ...
                : deltaX >= 0
                  ? 90
                  : 270;
        surveys.push({ md: cumulativeMD, inc, azi, comment: curr.comment });
    }
    return surveys;
}
...
function normalizeAnnulusFluidRow(row) {
    ...
    const placement =
        String(
            row.placement ?? row.Placement ?? FLUID_PLACEMENT_DEFAULT_OPTION,
        ).trim() || FLUID_PLACEMENT_DEFAULT_OPTION;
    const manualOD = parseOptionalNumber(
        row.manualOD ?? row.manual_od ?? row["Manual OD"],
    );
    const rawManualDepth = parseOptionalNumber(
        row.manualDepth ?? row.manual_depth ?? row["Label depth"],
    );
    const manualDepth =
        Number.isFinite(rawManualDepth) &&
        Number.isFinite(top) &&
        Number.isFinite(bottom) &&
        rawManualDepth >= top &&
        rawManualDepth <= bottom
            ? rawManualDepth
            : null;
    return {
        placement,
        ...
        color:
            row.color || row.Color || row["Fill color"] || DEFAULT_CEMENT_COLOR,
        hatchStyle: normalizeHatchStyle(
            row.hatch_style ||
                row.hatchStyle ||
                row.HatchStyle ||
                row["Hatch style"] ||
        ...
        fontSize:
            parseOptionalNumber(
                row.font_size ?? row.fontSize ?? row["Font size"],
            ) ?? 11,
        labelXPos: parseOptionalNumber(
            row.label_x ?? row.labelX ?? row.labelXPos ?? row["Label X"],
        ),
        manualDepth,
...
function resolveImportedWellConfig(baseConfig = {}, trajectoryRows = []) {
    ...
    if (!nextConfig.viewMode || String(nextConfig.viewMode).trim() === "") {
        nextConfig.viewMode =
            trajectoryRows.length >= 2 ? "directional" : "vertical";
    }
    return nextConfig;
}
...
function normalizeImportedProjectPayload(project = {}) {
    return {
        ...
        wells: (project.wells || []).map((well) => ({
            ...
            data: {
                ...well.data,
                userAnnotations: normalizeUserAnnotations(
                    well.data?.userAnnotations || [],
                ),
                annulusFluids: normalizeAnnulusFluids(
                    well.data?.annulusFluids || [],
                ),
...
function buildWellDataPayload(projectData = {}) {
    ...
    return {
        data: {
            casingData: withDefaultPipeComponentType(
                projectData.casingData || [],
            ),
            ...
            userAnnotations: normalizeUserAnnotations(
                projectData.userAnnotations || [],
            ),
            cementPlugs: projectData.cementPlugs || [],
            annulusFluids: normalizeAnnulusFluids(
                projectData.annulusFluids || [],
            ),
...
function buildBundledSampleData() {
    try {
        ...
        const payload = buildWellDataPayload(sampleWell.data);
        return { ...payload, config: cloneSnapshot(sampleWell.config || {}) };
    } catch (error) {
        console.warn(
            "Failed to parse bundled sample project; using legacy sample rows.",
            error,
        );
        return null;
    }
...
function loadSampleData(options = {}) {
    ...
    const sampleViewMode =
        String(bundledSample?.config?.viewMode || "").trim() || "vertical";
    const sampleConfig = resolveImportedWellConfig(
        { ...activeConfig, viewMode: sampleViewMode },
...
async function importExcelWorkbookFile(file, options = {}) {
    ...
    try {
        ...
        const projectData = await runWithWorkerFallback({
            ...
            runWorker: () =>
                importerWorker.parseStrictExcelProjectInWorker(buffer),
            runFallback: async () => {
...
async function importTrajectoryCsvFile(file, options = {}) {
    ...
    try {
        ...
        const trajectory = await runWithWorkerFallback({
            ...
            isCancelledError: importerWorker.isImporterWorkerCancelledError,
            runWorker: () =>
                importerWorker.parseTrajectoryCsvInWorker(csvString),
            runFallback: async () => {
        ...
        if (importMode === IMPORT_MODE_APPEND_NEW_WELL) {
            ...
            const nextData = cloneSnapshot(
                projectStore.activeWell?.data ?? createEmptyWellData(),
            );
...
async function importProjectJsonFile(file) {
    ...
    try {
        ...
        showAlert(
            `${t("alert.project_load_failed")}: ${err.message}`,
            "danger",
        );
        return false;
    }
...
async function importProjectJsonContent(content, fileContext = {}) {
    ...
    try {
        ...
        projectStore.setProjectFileContext(
            normalizeProjectFileContext(fileContext),
        );
        projectStore.syncActiveWellData();
        syncSelectionIndicators();
        showAlert(t("alert.project_loaded"), "success");
        ...
        showAlert(
            `${t("alert.project_load_failed")}: ${error.message}`,
            "danger",
        );
        return false;
    }
...
function appendSelectedWellsFromProjectPayload(project, selectedWellIds = []) {
    ...
    const targetWells = sourceWells.filter((well) =>
        selectedIds.has(String(well?.id ?? "").trim()),
    );
    let appendedCount = 0;
    targetWells.forEach((well) => {
        const result = projectStore.appendWell(
            { name: well?.name, data: well?.data, config: well?.config },
            { activate: false, ensureUniqueName: true },
        );
        if (result) {
            appendedCount += 1;
    ...
    if (appendedCount > 0) {
        showAlert(
            t("alert.wells_appended", { count: appendedCount }),
            "success",
        );
    }
