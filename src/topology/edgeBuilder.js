...
export function buildTerminationEdges(intervals, intervalNodeByKind, surfaceAssembly = null) {
    if (!Array.isArray(intervals) || intervals.length === 0) {
        return { nodes: [], edges: [], edgeReasons: {} };
    }

    const topInterval = intervals[0];
    const normalizedSurfaceAssembly = normalizeSurfaceAssembly(surfaceAssembly);
    if (normalizedSurfaceAssembly) {
        return buildSurfaceTerminationSubgraph(
            normalizedSurfaceAssembly,
            topInterval,
            intervalNodeByKind
        );
    }
    ...
    TOPOLOGY_VOLUME_KINDS.forEach((kind) => {
        ...
        const edgeId = createEdgeId(EDGE_KIND_TERMINATION, sourceNode.nodeId, 'node:SURFACE', kind);
        appendEdge(edges, edgeReasons, {
            edgeId,
...
function createSurfaceAssemblyNode(nodeId, kind, meta = {}) {
    return {
        nodeId,
        kind,
        depthTop: null,
        depthBottom: null,
        volumeKey: String(meta?.pathRoleKey ?? meta?.slotKey ?? kind),
        meta
    };
}

function parseSurfaceRouteToken(token) {
    const [rawEntityKind, ...rest] = String(token ?? '').split(':');
    const entityKind = String(rawEntityKind ?? '').trim().toLowerCase();
    const slotKey = rest.join(':').trim();
    if (!entityKind || !slotKey) return null;
    return { entityKind, slotKey };
}

function createSurfaceRouteNodeMaps(surfaceAssembly) {
    const nodes = [];
    const nodeByRouteToken = new Map();
    const entryPathByRoleKey = new Map();

    toSafeArray(surfaceAssembly?.entryPaths).forEach((entryPath) => {
        const roleKey = String(entryPath?.roleKey ?? '').trim();
        if (!roleKey) return;
        const routeToken = `entry:${roleKey}`;
        nodeByRouteToken.set(routeToken, createSurfaceAssemblyNode(
            `node:SURFACE_ENTRY:${roleKey}`,
            'SURFACE_ENTRY',
            {
                surfaceEntityKind: 'entry',
                pathRoleKey: roleKey,
                label: String(entryPath?.label ?? roleKey).trim() || roleKey,
                sourceVolumeKey: String(entryPath?.sourceVolumeKey ?? '').trim() || null
            }
        ));
        entryPathByRoleKey.set(roleKey, entryPath);
    });

    toSafeArray(surfaceAssembly?.junctions).forEach((junction) => {
        const junctionKey = String(junction?.junctionKey ?? '').trim();
        if (!junctionKey) return;
        nodeByRouteToken.set(`junction:${junctionKey}`, createSurfaceAssemblyNode(
            `node:SURFACE_JUNCTION:${junctionKey}`,
            'SURFACE_JUNCTION',
            {
                surfaceEntityKind: 'junction',
                junctionKey,
                pathRoleKey: String(junction?.pathRoleKey ?? '').trim() || null,
                label: String(junction?.label ?? junctionKey).trim() || junctionKey
            }
        ));
    });

    toSafeArray(surfaceAssembly?.devices).forEach((device) => {
        const slotKey = String(device?.slotKey ?? '').trim();
        if (!slotKey) return;
        nodeByRouteToken.set(`device:${slotKey}`, createSurfaceAssemblyNode(
            `node:SURFACE_DEVICE:${slotKey}`,
            'SURFACE_DEVICE',
            {
                surfaceEntityKind: 'device',
                slotKey,
                pathRoleKey: String(device?.pathRoleKey ?? '').trim() || null,
                label: String(device?.label ?? slotKey).trim() || slotKey,
                typeKey: String(device?.typeKey ?? '').trim() || null,
                state: String(device?.state ?? '').trim() || null
            }
        ));
    });

    toSafeArray(surfaceAssembly?.boundaries).forEach((boundary) => {
        const slotKey = String(boundary?.slotKey ?? '').trim();
        if (!slotKey) return;
        nodeByRouteToken.set(`boundary:${slotKey}`, createSurfaceAssemblyNode(
            `node:SURFACE_BOUNDARY:${slotKey}`,
            'SURFACE_BOUNDARY',
            {
                surfaceEntityKind: 'boundary',
                slotKey,
                pathRoleKey: String(boundary?.pathRoleKey ?? '').trim() || null,
                label: String(boundary?.label ?? slotKey).trim() || slotKey,
                typeKey: String(boundary?.typeKey ?? '').trim() || null,
                state: String(boundary?.state ?? '').trim() || null
            }
        ));
    });

    toSafeArray(surfaceAssembly?.terminations).forEach((termination) => {
        const slotKey = String(termination?.slotKey ?? '').trim();
        if (!slotKey) return;
        nodeByRouteToken.set(`termination:${slotKey}`, createSurfaceAssemblyNode(
            `node:SURFACE_TERMINATION:${slotKey}`,
            'SURFACE_TERMINATION',
            {
                surfaceEntityKind: 'termination',
                slotKey,
                pathRoleKey: String(termination?.pathRoleKey ?? '').trim() || null,
                label: String(termination?.label ?? slotKey).trim() || slotKey,
                typeKey: String(termination?.typeKey ?? '').trim() || null
            }
        ));
    });

    nodeByRouteToken.forEach((node) => {
        nodes.push(node);
    });
...
function resolveSurfaceControlStateForEntity(entityKind, entity) {
    if (entityKind === 'device') {
        ...
        if (state === 'closed') {
            return {
                ...
                summary: `${entity?.label ?? entity?.slotKey ?? 'Surface device'} is closed and acts as a barrier on the surface route.`
            };
        }
        return {
            cost: 0,
            state: 'open',
            summary: `${entity?.label ?? entity?.slotKey ?? 'Surface device'} is open on the surface route.`
        };
    }
    ...
    if (entityKind === 'boundary') {
        ...
        if (state === 'sealed') {
            return {
                cost: 1,
                state: 'closed_failable',
                summary: `${entity?.label ?? entity?.slotKey ?? 'Surface boundary'} is sealed and acts as a continuity barrier.`
            };
        }
        return {
            cost: 0,
            state: state || 'open',
            summary: `${entity?.label ?? entity?.slotKey ?? 'Surface boundary'} is not blocking continuity to surface.`
        };
    }
    ...
    return {
        cost: 0,
        state: 'open',
        summary: 'Surface route continues through the configured junction.'
    };
}

function resolveSurfaceTerminationState(termination) {
    const definition = getSurfaceAssemblyTerminationDefinition(termination?.typeKey);
    const label = String(termination?.label ?? termination?.slotKey ?? 'Surface termination').trim() || 'Surface termination';
    if (definition?.typeKey === 'none') {
        return {
            cost: 1,
            state: 'missing',
            summary: `${label} is not defined, so the surface route remains blocked until a termination is specified.`
        };
    }
    if (definition?.blocksSurface === true) {
        return {
            cost: 1,
            state: 'closed_failable',
            summary: `${label} terminates the surface route with a blocked outlet configuration.`
        };
    }
    return {
        cost: 0,
        state: definition?.surfaceState ?? 'open',
        summary: `${label} provides an open surface outlet.`
    };
}

function buildSurfaceTerminationSubgraph(surfaceAssembly, topInterval, intervalNodeByKind) {
    const { nodes, nodeByRouteToken, entryPathByRoleKey } = createSurfaceRouteNodeMaps(surfaceAssembly);
    const edges = [];
    const edgeReasons = {};
    const topologyRoutes = getSurfaceAssemblyTopologyRoutes(surfaceAssembly);
    const connectedTerminationTokens = new Set();

    topologyRoutes.forEach((routeTokens, routeIndex) => {
        const normalizedRouteTokens = toSafeArray(routeTokens)
            .map((token) => String(token ?? '').trim())
            .filter(Boolean);
        if (normalizedRouteTokens.length === 0) return;

        const entryToken = normalizedRouteTokens[0];
        const entryDescriptor = parseSurfaceRouteToken(entryToken);
        const entryNode = nodeByRouteToken.get(entryToken) ?? null;
        if (entryDescriptor?.entityKind === 'entry' && entryNode) {
            const entryPath = entryPathByRoleKey.get(entryDescriptor.slotKey) ?? null;
            const sourceVolumeKey = String(entryPath?.sourceVolumeKey ?? '').trim();
            const sourceNode = sourceVolumeKey
                ? intervalNodeByKind.get(`${topInterval.intervalIndex}|${sourceVolumeKey}`) ?? null
                : null;
            if (sourceNode) {
                appendEdge(edges, edgeReasons, {
                    edgeId: createEdgeId(
                        ...
                        sourceNode.nodeId,
                        entryNode.nodeId,
                        `surface-entry-slot:${entryDescriptor.slotKey}:${routeIndex}`
                    ),
                    from: sourceNode.nodeId,
                    to: entryNode.nodeId,
                    kind: EDGE_KIND_TERMINATION,
                    ...
                    meta: {
                        ...
                        pathRoleKey: entryDescriptor.slotKey,
                        surfaceEntityKind: 'entry'
                    },
                    reason: {
                        ruleId: 'surface-entry-anchor',
                        summary: `${entryPath?.label ?? entryDescriptor.slotKey} enters the configured surface assembly.`,
                        details: {
                            intervalIndex: topInterval.intervalIndex,
                            sourceVolumeKey,
                            pathRoleKey: entryDescriptor.slotKey
        ...
        for (let index = 0; index < normalizedRouteTokens.length - 1; index += 1) {
            const fromToken = normalizedRouteTokens[index];
            const toToken = normalizedRouteTokens[index + 1];
            const fromNode = nodeByRouteToken.get(fromToken) ?? null;
            const toNode = nodeByRouteToken.get(toToken) ?? null;
            const toDescriptor = parseSurfaceRouteToken(toToken);
            if (!fromNode || !toNode || !toDescriptor) continue;

            const entityKey = `${toDescriptor.entityKind}:${toDescriptor.slotKey}`;
            const entity = toDescriptor.entityKind === 'junction'
                ? null
                : resolveSurfaceAssemblyEntity(surfaceAssembly, entityKey);
            const transition = resolveSurfaceControlStateForEntity(toDescriptor.entityKind, entity);
            ...
            appendEdge(edges, edgeReasons, {
                edgeId: createEdgeId(
                    ...
                    fromNode.nodeId,
                    toNode.nodeId,
                    `surface-${toDescriptor.entityKind}-slot:${toDescriptor.slotKey}:${routeIndex}:${index}`
                ),
                from: fromNode.nodeId,
                to: toNode.nodeId,
                kind: EDGE_KIND_VERTICAL,
                ...
                meta: {
                    surfaceEntityKind: toDescriptor.entityKind,
                    slotKey: toDescriptor.slotKey
                },
                reason: {
                    ruleId: `surface-${toDescriptor.entityKind}-continuity`,
        ...
        const lastToken = normalizedRouteTokens[normalizedRouteTokens.length - 1];
        const terminationDescriptor = parseSurfaceRouteToken(lastToken);
        const terminationNode = nodeByRouteToken.get(lastToken) ?? null;
        if (
            terminationDescriptor?.entityKind !== 'termination'
            || !terminationNode
            || connectedTerminationTokens.has(lastToken)
        ) {
            return;
        }

        const termination = resolveSurfaceAssemblyEntity(
            surfaceAssembly,
            `termination:${terminationDescriptor.slotKey}`
        );
        const terminationState = resolveSurfaceTerminationState(termination);
        appendEdge(edges, edgeReasons, {
            edgeId: createEdgeId(
                EDGE_KIND_TERMINATION,
                terminationNode.nodeId,
                'node:SURFACE',
                `surface-termination-slot:${terminationDescriptor.slotKey}`
            ),
            from: terminationNode.nodeId,
            to: 'node:SURFACE',
            kind: EDGE_KIND_TERMINATION,
            cost: terminationState.cost,
            state: terminationState.state,
            meta: {
                surfaceEntityKind: 'termination',
                slotKey: terminationDescriptor.slotKey,
                typeKey: String(termination?.typeKey ?? '').trim() || null
            },
            reason: {
                ruleId: 'surface-termination-slot',
                summary: terminationState.summary,
                details: {
                    routeIndex,
                    slotKey: terminationDescriptor.slotKey
                }
        ...
        connectedTerminationTokens.add(lastToken);
    });
