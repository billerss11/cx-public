...
const ContextMenuStub = defineComponent({
  ...
  setup(props, { expose }) {
    expose({
      show: () => {
        const deleteAction = toSafeArray(props.model).find((entry) => entry?.label === 'Delete');
...
describe('LeftHierarchyDock delete context menu', () => {
  it('deletes item row via right-click context menu', async () => {
      ...
      wells: [
        {
          ...
          name: 'Well 1',
          data: createWellData([
            { rowId: 'line-1', depth: 1200, label: 'Delete Me', show: true }
          ]),
          config: {}
        }
      ]
